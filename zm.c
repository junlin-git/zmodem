...
void zm_set_zctlesc(zm_t *zm, int x)
{
    zm->zctlesc = x;
}
 ...
 */
static inline int zm_get_ascii_char(zm_t *zm)
{
    int c;
 ...
 */
static inline int zm_get_escaped_char(zm_t *zm)
{
...
static int zm_get_escaped_char_internal(zm_t *zm, int c)
{
...
again2:
    ...
    log_debug("Bad escape sequence %x", c);
    return ERROR;
}
...
void zm_put_escaped_char(zm_t *zm, int c)
{
    ...
    case ZM_ESCAPE_NEVER:
        zreadline_write(zm->lastsent = c);
        break;
    case ZM_ESCAPE_ALWAYS:
        zreadline_write(ZDLE);
        /* Spec 7.2: The receiving program decodes any sequence
         * of ZDLE followed by a byte with bit 6 set and bit 5 reset
         * (uppercase letter, either parity) to the equivalent control
         * character by inverting bit 6 */
...
static void zm_put_escaped_string (zm_t *zm, const char *s, size_t count)
{
 ...
 */
int zm_receive_data(zm_t *zm, char *buf, int length, size_t *bytes_received)
{
    int c;
    unsigned short crc;
    int d;
...
static int zm_read_data32(zm_t *zm, char *buf, int length, size_t *bytes_received)
{
    int c;
    unsigned long crc;
    int d;
...
void zm_escape_sequence_init(zm_t *zm)
{
